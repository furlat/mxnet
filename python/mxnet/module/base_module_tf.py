...
class BaseModule(object):
    ...
    def fit(self, train_data, eval_data=None, eval_metric='acc',
            ...
            validation_metric=None, monitor=None, log_prefix=None, multi_data=None):
        ...
        if validation_metric is None:
            validation_metric = eval_metric[0]
        #if not isinstance(eval_metric, metric.EvalMetric):
         #   eval_metric = metric.create(eval_metric)
        global_batch_id = 0
        log_glob_batchid=[]
        ...
        for epoch in range(begin_epoch, num_epoch):
            log_batchid=[]
            log_metric = [[] for i in range(len(eval_metric))]   
            tic = time.time()
            for metric in eval_metric:
                metric.reset()
            for nbatch, data_batch in enumerate(train_data):
                if data_batch.bucket_key is not None:
                    bkey=data_batch.bucket_key
                else:
                    bkey=0
                if monitor is not None:
                    monitor.tic()
                ...
                self.update()
                self.update_metric(eval_metric[bkey], data_batch.label)
                if nbatch % 50 == 0:
                    log_glob_batchid.append(global_batch_id)
                    log_batchid.append(nbatch)                    
                    for i, metric in enumerate(eval_metric):                                               
                        val = metric.get_name_value()                        
                        log_metric[i].append([value[1] for value in val])                        
                ...
                #print ['[%d] Eval metric = %s for bucket number %d' %(nbatch, eval_metric.sum_metric/data_batch.provide_data[0][1][0])] 
                #print ['[%d] Eval metric = %s for bucket number ' %(nbatch, eval_metric.sum_metric/64)]
                #for metrica in eval_metric.sum_metric:
                 #   print '[%d] Entropy %s ' %(nbatch, metrica/train_data.provide_data[0][1][0])  
            ...
            if multi_data is not None:                
                self.switch_bucket(self._default_bucket_key, data_batch.provide_data,
            ...
            #    self._buckets[self._default_bucket_key].save_params(fname)
                
            #----------------------------------------
            # evaluation on validation set
            if eval_data:
                if multi_data is not None:
                    for buck,eval_multidata in enumerate(eval_data):
                        res = self._buckets[buck].score(eval_multidata, validation_metric,
            ...
            for j, log in enumerate(log_metric):
                logfile_url= '%s-metric-log-%01d.txt' % (log_prefix,j)
                print 'saving logfiles%01d  at %s' % (j,logfile_url)
                logfile = open(logfile_url, 'a')
                for  bnum,bglobnum, bmetric in zip(log_batchid,log_glob_batchid,log):
                    #logfile.write("%s\n" % item)
                    if len(bmetric)>1:

                        all_metrics = '\t'.join(str(single_metr) for single_metr in bmetric)
                    elif len(bmetric)==1:
                        all_metrics=str(bmetric[0])
                    logfile.write(str(bglobnum)+"\t"+str(epoch)+"\t"+ str(bnum)+"\t"+str(i)+"\t"+ all_metrics +"\n")
                logfile.close()
