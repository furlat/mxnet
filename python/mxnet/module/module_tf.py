...
class Module(BaseModule):
    ...
    def bind(self, data_shapes, label_shapes=None, for_training=True,
        ...
        if shared_module is not None: #only subset?
            self.params_initialized = True
            self._arg_params = {name:[] for name in self._param_names}
            self._aux_params = {name:[] for name in self._aux_names}
            for name in self._param_names:
                
                self._arg_params[name] = shared_module._arg_params[name]
            for name in self._aux_names: 
                self._aux_params[name] = shared_module._aux_params[name]    
    ...
    def init_optimizer(self, kvstore='local', optimizer='sgd',
        ...
        #create a key_dict for 
        key_dict = {name:[] for name in self._param_names}
    ...
    def update(self):
        ...
        if self._update_on_kvstore:
            ...
            _update_params_on_kvstore_with_keys(self._exec_group.param_arrays,
                                       self._exec_group.grad_arrays,
                                       self._kvstore, self.keys_dict, self._exec_group.param_names)
            # _update_params_on_kvstore(self._exec_group.param_arrays,
            #                           self._exec_group.grad_arrays,
            #                           self._kvstore)
            #print len(self._exec_group.grad_arrays), len(self._exec_group.param_names), len(self.keys_dict)
            #print 'yoo'
        else:
