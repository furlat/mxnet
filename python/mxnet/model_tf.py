...
def _create_kvstore(kvstore, num_device, arg_params):
    ...
    if kvstore is None:
        ...
        if num_device is 0 and 'dist' not in kvstore: #switched from 0 to 1 to force kv store with 1 gpu
...
def _initialize_kvstore_out_keys(kvstore, param_arrays, arg_params, param_names,
                        update_on_kvstore):
    """ Initialize kvstore"""
    key_dict = {name:[] for name in param_names}
    for idx, param_on_devs in enumerate(param_arrays):
        kvstore.init(idx, arg_params[param_names[idx]])
        keys_dict[name]=idx
...
def _update_params_on_kvstore_with_keys(param_arrays, grad_arrays, kvstore, keys_dict, param_names):
    """ Perform update of param_arrays from grad_arrays on kvstore."""
    for pair in zip(param_arrays, grad_arrays, param_names):
        arg_list, grad_list, name = pair
        index = keys_dict[name]
        if grad_list[0] is None:
            continue
        # push gradient, priority is negative index
        kvstore.push(index, grad_list, priority=-index)
